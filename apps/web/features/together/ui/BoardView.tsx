...
export function BoardView({
  ...
  const filterTasks = (tasks: TogetherTask[]): TogetherTask[] => {
    switch (filter) {
      ...
      case "together":
        return tasks.filter((task) => task.assigneeMode === "BOTH");

      case "rotate":
        return tasks.filter((task) => task.assigneeMode === "ROTATE");
   ...
   * - после запроса синхронизируем board с backend.
   */
  const moveTaskMutation = useMutation({
    ...
    onMutate: async ({ taskId, columnId, order }) => {
       ...
       * Защита от undefined.
       *
       * Именно здесь раньше TypeScript терял обязательный `id`.
       */
    ...
    onError: (_error, _variables, context) => {
      if (context?.previousBoard) {
        queryClient.setQueryData<Board>(togetherKeys.board(), context.previousBoard);
      }
    },
  ...
  const reorderColumnsMutation = useMutation({
    ...
    onMutate: async (orderedIds) => {
      ...
      queryClient.setQueryData<Board>(togetherKeys.board(), (old) => {
        if (!old) return old;
        const idToCol = new Map(old.columns.map((c) => [c.id, c]));
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    if (overType === "column") {
      ...
      /**
       * Если бросили в ту же колонку, добавляем в конец.
       */
      const targetOrder =
    ...
    if (overType === "task") {
      ...
      /**
       * Если таска двигается внутри той же колонки
       * вниз, после удаления исходной позиции
       * индекс назначения уменьшается на 1.
       */
