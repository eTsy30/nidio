...
const messages = [
  { title: "Сегодня свободно", text: "Может, запланируете что-нибудь вместе?" },
  { title: "День без планов", text: "Иногда лучший момент — тот, который вы придумали сами." },
  {
    title: "Можно придумать что-нибудь",
    text: "Даже маленький момент может стать вашим воспоминанием.",
  },
  { title: "Время для вас", text: "Почему бы не добавить что-нибудь особенное?" },
];

export function EmptyDayMessage({ date }: { date: Date }) {
  const message = useMemo(() => messages[Math.abs(date.getDate()) % messages.length], [date]);
  return (
